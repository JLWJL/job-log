...
export default class NewJob extends React.Component {
  ...
  handleSubmit (event) {
    ...
    this.jobService.newJob(jsonFormData).then(
      res => {
  ...
  render () {
    return (
      <div>
        <h2>New Job</h2>
        <hr/>
        <form id="new-job-form" onSubmit={this.handleSubmit}
          ...
          <div className="form-group">
            ...
            <textarea name="other" className="form-control" id="other" cols="30"
                      rows="10" onChange={this.handleChange}
                      value={this.state.other}></textarea>
          </div>

          <button type="submit" className="btn btn-primary"
                  onClick={(e) => {this.handleSubmit(e);}}>OK
          </button>
